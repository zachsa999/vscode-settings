...
$altweekFolderName = 'week_' + (get-date -UFormat %V) + '_' + (Get-Date -Format "yyyy") + '1' + '\'
...
function MoveFiles {
    ...
    if (!(Test-Path -Path ($weekFolderName))) {
        ...
        New-Item -Path ($weekFolderName + 1) -ItemType Directory
        OutputPS ("folder does not exist Creating folder " + $weekFolderName + _1)
    }
