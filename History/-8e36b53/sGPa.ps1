...
function MoveFiles {
    ...
    Set-Location -Path $rootPathSet-Location -Path $rootPath
...
function MoveRest {
    Set-Location -Path $rootPath + $weekFolderName
    Get-ChildItem -File | ForEach-Object {
        # extract metadata
        $fileName = $_.ToString()
...
function MergeFiles {
    ...
    Get-ChildItem -Directory | ForEach-Object {
        ...
        # Write-Output $files    
        Set-Location $_
        magick mogrify -format pdf *.jpeg
        Set-Location $rootPath + $weekFolderName
